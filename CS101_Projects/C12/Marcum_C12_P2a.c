...
//  Purpose: Checks a message if it's a palindrome or not - Int Version
//  Author: James Marcum
...
//bool palindrome_check_point(char mess[], int num); - Not included in this version
...
int message_len = read_mess(message, MAX);
	...
	//call palindrome_check_int and display result
	if (palindrome_check_int(message,message_len))
		printf("\nIt's really a palindrome!\n");
	else
		printf("\nThat was NOT a palindrome!\n");

	//call palindrome_check_point and display result - NOT INCLUDED IN THIS VERSION
	/*
...
char input;
	...
	//if it is anything between A-Z add it to the array and increase i
	while ((input = getchar()) != '\n' && i < num)
		if (isalpha(input))
			{
